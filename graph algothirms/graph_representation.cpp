...
void add_edge(int u,int v)
    {nxt[++ecnt]=fir[u];to[ecnt]=v;fir[u]=ecnt;}
void remove(int u,int v)
{
    if(to[fir[u]]==v){fir[u]=nxt[fir[u]];return;}
    for(int i=fir[u];i;i=nxt[i])if(to[nxt[i]]==v)
        {nxt[i]=nxt[nxt[i]];return;}
}
void prevs(int u,int pre,int& cur)
{
    cur++;sz[u]=1;
    for_edges(u,v)if(v!=pre)
        t[v]=u,prevs(v,u,cur),sz[u]+=sz[v];
}

ll res=0;
int d[mn],on[21],off[21];vector<int>ds;
...
//find the centroid,then solve the problems concerning the subtree containing u
{
    ...
    int cur_sz=0;prevs(u,pre,cur_sz);
    while(1)
    {
        int cand=-1;
        for_edges(u,v)
        ...
        if(cand!=-1)pre=u,u=cand;else break;
    }
    d[u]=0;res+=a[u];
    for_edges(u,v)
    {
        ...
        for(int dist:ds)
        {
            int cur=dist^a[u];res+=cur;
            F(i,0,20)if(cur&(1<<i))res+=(ll)off[i]*(1<<i);
                else res+=(ll)on[i]*(1<<i);
        }
        for(int dist:ds)F(i,0,20)
            if(dist&(1<<i))on[i]++;else off[i]++;
    }
