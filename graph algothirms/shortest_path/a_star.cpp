...
void AStar(int a[100][100], int n, int start, int finsh, int b[]) {
    ...
    while (Count(l, Open) !=
           0)  // kiem tra xem tap Open co con phan tu nao khong
    {
        ...
        if (FindPoint(n, p, Open[k].index) == finsh) {
            ...
            for (int i = 0; i < n; i++) {
                if (a[FindPoint(n, p, Open[k].index)][i] != 0 &&
                    p[i].color == 0)  // neu chua co trong Open va Close
                {
                    Open[l] = p[i];
                    Open[l].color = 1;
                    Open[l].h = a[FindPoint(n, p, Open[k].index)][i] +
                                Open[k].h;  // tinh h khoang cach ngan nhat tu
                                            // dinh bat dau den dinh hien tai
                    ...
                    p[i].color == 1)  // neu dinh da co trong Open
                {
                    ...
                    tempNode.color = 1;
                    tempNode.h =
                        a[FindPoint(n, p, Open[k].index)][i] + Open[k].h;
                    tempNode.parent = k;
                    tempNode.f = tempNode.h + tempNode.g;
                    if (tempNode.f <
                        Open[h].f)  // neu f trang thai hien tai be hon trang
                                    // thai cap nhat truoc do
                        Open[h] = tempNode;
                }
                if (a[FindPoint(n, p, Open[k].index)][i] != 0 &&
                    p[i].color == 2)  // neu dinh da co trong Close
                {
                    ...
                    tempNode.parent = k;
                    tempNode.f = tempNode.h + tempNode.g;
                    if (tempNode.f < Close[h].f)  // neu f trang thai hien tai
                                                  // be hon trang thai truoc do
                    {
