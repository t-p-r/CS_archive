...
class segment_tree
{
	...
	void point_update(int id,int l,int r,int i,int val) 
    {
        down(id,l,r);
		if(i<l || r<i)return;
        if(l==r){lo[id]=hi[id]=val;return;}
        point_update(lc,i,val);point_update(rc,i,val);
		lo[id]=min(lo[2*id],lo[2*id+1]);
        hi[id]=max(hi[2*id],hi[2*id+1]);
    }
	...
	{
		LB=l,RB=r;
		lo.resize(4*(r-l+1)+7,inf);hi.resize(4*(r-l+1)+7,-inf);
		lazy.resize(4*(r-l+1)+7,0);
	}
	void point_update(int i,int val)
		{point_update(1,LB,RB,i,val);}
	void range_update(int u,int v)
		{range_update(1,LB,RB,u,v);}
    int get(int u,int v)
		{return u>v ? -inf : get(1,LB,RB,u,v);}
};
vector<int>a[mn];
int n,q;
int t[mn],h[mn],sz[mn];
...
void prevs(int u,int pre)
{
	...
	for(ii v:adj[u])if(v.fi!=pre)
		p[v.fi][0]=t[v.fi]=u,h[v.fi]=h[u]+1,
        prevs(v.fi,u),sz[u]+=sz[v.fi];
}
int cnt=1;
void dfs(int u,int pre,int len)
{
	tc[u]=len;
	a[chain[u]=cnt].pb(len),
	edge_id[u]=a[cnt].size()-1;
	int idx=-1,cur=-1,clen=-inf;
	for(ii v:adj[u])if(v.fi!=pre && cur<sz[v.fi])cur=sz[v.fi],idx=v.fi,clen=v.se;
	if(idx!=-1)dfs(idx,u,clen);
	for(ii v:adj[u])if(v.fi!=pre && v.fi!=idx)head[++cnt]=v.fi,dfs(v.fi,u,v.se);
}
...
int dir(int u,int v){return adv(v,h[v]-h[u]-1);}
void solve()
{
	int n;cin>>n;cnt=1;
	memset(p,0,sizeof(p));
	...
	string s;while(cin>>s)
	{
		...
		{
			int i,val;cin>>i>>val;
			int u,v;tie(u,v)=e[i];if(h[u]>h[v])swap(u,v);
		...
		else
		{
			...
			if(s=="QUERY")
			{
				...
				else
				{
					...
					if(u!=k)cur=max(cur,st[chain[u]].get(edge_id[dir(k,u)],edge_id[u]));
					...
					if(v!=k)cur=max(cur,st[chain[v]].get(edge_id[dir(k,v)],edge_id[v]));
			...
			else if(u!=v)
			{
				...
				if(u!=k)st[chain[u]].range_update(edge_id[dir(k,u)],edge_id[u]);
				...
				if(v!=k)st[chain[v]].range_update(edge_id[dir(k,v)],edge_id[v]);
			}
		}
	}
}
