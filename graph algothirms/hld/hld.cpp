...
void prevs(int u, int pre) {
    sz[u] = 1;
    for (int v : adj[u])
        if (v != pre) t[v] = u, h[v] = h[u] + 1, prevs(v, u), sz[u] += sz[v];
}
int cnt = 1;
void dfs(int u, int pre) {
    ...
    int idx = -1, cur = -1;
    for (int v : adj[u])
        if (v != pre && cur < sz[v]) cur = sz[v], idx = v;
    if (idx != -1) dfs(idx, u);
    for (int v : adj[u])
        if (v != pre && v != idx) ++cnt, dfs(v, u);
}
...
int32_t main() {
    ...
    F(i, 1, q) {
        ...
        if (type) {
            ...
            int i = next_l();
            if (!on[i])
                blacks[chain[i]].insert({h[i], i});
            else
                blacks[chain[i]].erase({h[i], i});
            on[i] ^= 1;
        }
    }
}