...
//This BFS partitions the graph into layers
    ...
    since the free vertices in U are by definition not adjacent to any matched edges.*/
    while(!q.empty()){
        int u=q.front();q.pop();
        if(d[u]<d[0])
            for(int v:adj[u])if(d[pv[v]]==inf)
                d[pv[v]]=d[u]+1,q.push(pv[v]);
        /* At subsequent levels of the search, the traversed edges are required to alternate 
        between matched and unmatched. 
        That is, when searching for successors from a vertex in U, only unmatched edges 
        may be traversed, 
