...
{
    F(i,p,n,lsb(i)){min1[i]=min(min1[i],val);max1[i]=max(max1[i],val);}
    D(i,p,1,lsb(i)){min2[i]=min(min2[i],val);max2[i]=max(max2[i],val);}
}

int get_min(int l,int r)
{
    int ans=inf,i=l;
    while(i+lsb(i)<=r){ans=min(ans,min2[i]);i+=lsb(i);}
    i=r;
    while(i-lsb(i)>=l){ans=min(ans,min1[i]);i-=lsb(i);}
    return min(ans,a[i]);
}
...
{
    int ans=-inf,i=l;
    while(i+lsb(i)<=r){ans=max(ans,max2[i]);i+=lsb(i);}
    i=r;
    while(i-lsb(i)>=l){ans=max(ans,max1[i]);i-=lsb(i);}
    return max(ans,a[i]);
}
