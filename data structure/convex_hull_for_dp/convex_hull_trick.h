...
int query(int x) {
    ...
    while (j + 1 < a.size() && eval(j + 1, x) < eval(j, x))
        j++;
    return eval(j, x);
}
// CONCEPT REQUIREMENT: a is non-increasing (if query finds minimum value) and
// vice versa; j is non-decreasing If concept requirements are not met, use a Li
// Chao tree instead.
}  // namespace convex_hull_trick
