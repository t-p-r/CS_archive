...
#define rc 2 * id + 1, mid + 1, r
   private:
    ...
    void update(int id, int l, int r, int u, int v, line val) {
        if (v < l || r < u) return;
        if (u <= l && r <= v) {
            if (f(st[id], l) >= f(val, l) && f(st[id], r) >= f(val, r)) return;
   ...
   public:
    void insert(line nw, int u, int v) { update(1, LB, RB, u, v, nw); }
    int get(
        int i)  // the line which yields the lowest y_value with given x_value
    {
