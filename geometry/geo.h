...
namespace geo
{
    ...
    struct point 
    {
        coord x,y;
        point operator-()
        {point res=*this;res.x=-res.x,res.y=-res.y;return res;}
        point operator=(const point& k)
        {this->x=k.x,this->y=k.y;return *this;}
    ...
    std::vector<point> graham_scan(std::vector<point> p)
    {
        ...
        int i=0,n=p.size();
        F(j,1,n-1)if(p[j].y<p[i].y || p[j].y==p[i].y && p[j].x<p[i].x)i=j;
        swap(p[i],p[0]);a=p[0];
        ...
        int m=0;std::vector<point>q;
        F(i,0,n-1)
        {
            while(m>=2 && !ccw(q[m-2],q[m-1],p[i]))q.pop_back(),--m;
            q.pb(p[i]),m++;
        }
        while(m>=2 && !ccw(q[m-2],q[m-1],q[0]))q.pop_back(),m--;
        return q;
        
    }
    int area(std::vector<point>p)
    {
        int n=p.size(),res=0;p.pb(p[0]);
