...
int gauss(vector<vector<int>>a,vector<int>& ans){
	...
	for(int col=0,row=0;col<m && row<n;col++){
		int pos=row;
		for(int i=row;i<n;i++)if(abs(a[i][col])>abs(a[pos][col]))pos=i;

		if(abs(a[pos][col])<EPS)continue;
		// no pivot row found, x[col] is an independant variable
		a[pos].swap(a[row]);where[col]=row;
	...
	// now that we have found all variables that we can, simply ignore all other variables
	// (i.e let them equates 0), and see if the variables that we found can be a solution
	// to the SLAE
