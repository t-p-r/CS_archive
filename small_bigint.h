...
class bigint: public vector<int>{
    ...
    bigint(const string& s){
        for(int i=s.size();i>0;i-=9){
            if(i<9)this->emplace_back(atoi(s.substr(0,i).c_str()));
            else this->emplace_back(atoi(s.substr(i-9,9).c_str()));
        }
    }
    bigint(int s){
        string v;
        while(s)v+=(s%10+48),s/=10;
        reverse(allof(v));
        *this=v;
    }
    ...
    friend ostream& operator<<(ostream &os, const bigint &v){
        os<<(v.empty() ? 0 : v.back());
        D(i,(signed)v.size()-2,0)os<<setw(base_digits)<<setfill('0')<<v[i];
        return os;
    }
